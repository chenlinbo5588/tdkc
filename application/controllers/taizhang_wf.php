...
class Taizhang_Wf extends TZ_Admin_Controller {
    ...
    public function add(){
        ...
        if($this->isPostRequest()){
            ...
            if($this->form_validation->run()){
                ...
                for($i = 0 ; $i < 1; $i++){
                    if('保存' == $_POST['submit']){
                        $dupTips = $this->_getDupList(TAIZHANG_WF, $_POST['name']);
                        if($dupTips){
                            $info = $_POST;
                            $this->assign('saveText','确认');
                            $this->assign('dupTips','<p>名称'.$_POST['name'].'已入'.TAIZHANG_WF.'台账，点击<span class=\"notice\">确定</span>继续保存，</p><p>原台账号信息:</p>'.  implode('', $dupTips));
                            break;
                        }
                    }
    ...
    private function _op($year,$action = 'add'){
        ...
        if($action == 'add'){
            ...
            if(!empty($offset)){
                $_POST['master_serial'] += (int)$offset['WF_TOTAL'];
                $_POST['region_serial'] += (int)$offset['WF'][$_POST['region_code']];
            }
    ...
    private function _initPageData($addYear){
        ...
        $projectTypeList = $this->Project_Type_Model->getList(array(
           'where_in' => array(
               array('key' => 'name','value' => array('日常宗地'))
           ),
