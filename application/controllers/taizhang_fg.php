        ...
        if($this->isPostRequest()){
            ...
            if($this->form_validation->run()){
                ...
                for($i = 0 ; $i < 1; $i++){
                    ...
                    if('保存' == $_POST['submit']){
                        $dupTips = $this->_getDupList(TAIZHANG_FG, $_POST['name']);
                        if($dupTips){
                            $info = $_POST;
                            $this->assign('saveText','确认');
                            $this->assign('dupTips','<p>名称'.$_POST['name'].'已入'.TAIZHANG_FG.'台账，点击<span class=\"notice\">确定</span>继续保存，</p><p>原台账号信息:</p>'.  implode('', $dupTips));
                            break;
                        }
                    }
