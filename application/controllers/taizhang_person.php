...
class Taizhang_Person extends TZ_Admin_Controller {
    ...
    private function _formatProjectNo($regionCode,$regionSerial,$prefix = ''){
        
        if($regionSerial < 1000){
            $regionSerial = str_pad($regionSerial, 3,'0', STR_PAD_LEFT);
        }
        
        return $prefix.strtoupper($regionCode).$regionSerial;
    }
    ...
    public function add(){
        ...
        if($this->isPostRequest()){
            ...
            if($this->form_validation->run()){
                ...
                for($i = 0 ; $i < 1; $i++){
                    if('保存' == $_POST['submit']){
                        $dupTips = $this->_getDupList(TAIZHANG_PERSON, $_POST['name']);
                        if($dupTips){
                            $info = $_POST;
                            $this->assign('saveText','确认');
                            $this->assign('dupTips','<p>名称'.$_POST['name'].'已入'.TAIZHANG_PERSON.'台账，点击<span class=\"notice\">确定</span>继续保存，</p><p>原台账号信息:</p>'.  implode('', $dupTips));
                            break;
                        }
                    }
                    ...
                    $_POST['year'] = date("Y");
                    $_POST['month'] = date("m");
                    $_POST['category'] = TAIZHANG_PERSON;
    ...
    private function _op($year,$action = 'add'){
        ...
        if($action == 'add'){
            ...
            if(!empty($offset)){
                $_POST['master_serial'] += (int)$offset['PERSON_TOTAL'];
                $_POST['region_serial'] += (int)$offset['PERSON'][$_POST['region_code']];
            }
            
            $_POST['project_no'] = $this->_formatProjectNo($_POST['region_code'],$_POST['region_serial']);
        }
