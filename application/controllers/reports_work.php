...
class reports_work extends TZ_Admin_Controller {
    ...
    private function _report_work(){
        ...
        $cd = array(
            ...
            'order' => 'pm ASC , createtime ASC , type ASC , region_code ASC'
        );
        ...
        */
        ...
        $objPHPExcel->getActiveSheet()->setCellValue('A1', $_POST['sdate'].'至'.$_POST['edate'].'小组工作量统计');
        ...
        $objPHPExcel->getActiveSheet()->setCellValue('H2', '主要完成人');
        $objPHPExcel->getActiveSheet()->setCellValue('I2', '工期要求');
        $objPHPExcel->getActiveSheet()->setCellValue('J2', '起始时间');
        $objPHPExcel->getActiveSheet()->setCellValue('K2', '结束时间');
        $objPHPExcel->getActiveSheet()->setCellValue('L2', '完成情况');
        $objPHPExcel->getActiveSheet()->setCellValue('M2', '资料领取');
        $objPHPExcel->getActiveSheet()->setCellValue('N2', '业主附加要求');
        ...
        foreach($projectList['data'] as $p){
            ...
            $objPHPExcel->getActiveSheet()->setCellValue('I'.$current_row, ceil(($p['end_date'] - $p['start_date'])/86400) + 1) ;
            $objPHPExcel->getActiveSheet()->setCellValue('J'.$current_row, date("Y-m-d",$p['start_date']));
            $objPHPExcel->getActiveSheet()->setCellValue('K'.$current_row, date("Y-m-d",$p['end_date']));
            $objPHPExcel->getActiveSheet()->setCellValue('L'.$current_row, '');
            $objPHPExcel->getActiveSheet()->setCellValue('M'.$current_row, '');
            $objPHPExcel->getActiveSheet()->setCellValue('N'.$current_row, $p['descripton']);
        ...
        $objPHPExcel->getActiveSheet()->getStyle('A1:N'.(count($projectList['data']) + 2))->applyFromArray(
                array(
                    'alignment' => array(
