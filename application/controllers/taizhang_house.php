...
class Taizhang_House extends TZ_Admin_Controller {
    ...
    private function _addRules(){
        ...
        $this->form_validation->set_rules('name', '项目名称', 'trim|required|max_length[200]|htmlspecialchars');
        ...
        $this->form_validation->set_rules('complete_time', '出报告时间', 'required|valid_date');
    ...
    public function add(){
        ...
        if($this->isPostRequest()){
            ...
            $this->_fetchProjectInfo(TAIZHANG_HOUSE);
        }
    ...
    private function _op($year,$action = 'add'){
        ...
        if($action == 'add'){
            ...
            if(!empty($offset)){
                $_POST['master_serial'] += (int)$offset['HOUSE_TOTAL'];
            }
    ...
    private function _initPageData($addYear){
        ...
        $projectTypeList = $this->Project_Type_Model->getList(array(
           'where_in' => array(
               array('key' => 'cate_name','value' => array('房产测绘'))
           ),
