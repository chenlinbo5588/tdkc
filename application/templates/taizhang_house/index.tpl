{include file="common/main_header.tpl"}
            <div class="row-fluid">
                ...
                <form action="{url_path('taizhang_house')}" method="get" name="searchform">
                    <input type="hidden" value="taizhang_house" name="{config_item('controller_trigger')}"/>
                    <input type="hidden" value="{$action}" name="{config_item('function_trigger')}"/>
                    <ul>
                        <li>
                            <label><strong>合同编号</strong><input type="text" name="project_no" style="width:150px;" value="{$smarty.get.project_no}" placeholder="请输入合同编号"/></label>
                            ...
                            {auth name="taizhang_house+add"}<a href="javascript:void(0);" class="addlink">+新增房产项目台账</a>{/auth}
                        </li>
                     </ul>
                </form>
            </div>
            ...
            <div class="span12">
                {auth name="taizhang_house+delete"}
                <div class="operator">
                ...
                <table class="table" id="listtable" >
                    <thead>
                        <tr>
                            {auth name="taizhang_house+delete"}<th></th>{/auth}
                            <th>时间</th>
                            <th>项目名称</th>
                            <th>土地坐落</th>
                            <th>作业组负责人</th>
                    ...
                    <tbody>
                        ...
                        <tr id="row_{$item['id']}">
                           {auth name="taizhang_house+delete"}<td class="center"><input type="checkbox" name="id[]" value="{$item['id']}"/></td>{/auth}
                           ...
                           <td>
                               {auth name="taizhang_house+edit"}<a href="javascript:void(0);" class="edit" data-id="{$item['id']}" data-href="{url_path('taizhang_house','edit','id=')}{$item['id']}">编辑</a>{/auth}
                               {auth name="taizhang_house+fee"}<a href="javascript:void(0);" class="popwin" data-id="{$item['id']}" data-href="{url_path('taizhang_house','fee','id=')}{$item['id']}">收费</a>{/auth}
                           </td>
                        </tr>
                        {/foreach}
                    </tbody>
            ...
            <form id="delete_form" name="deleteForm" action="{url_path('taizhang_house','delete')}" method="post" target="post_iframe">
                <div class="inputlist">
                 ...
                 $(function(){
                    ...
                    $("#listtable").delegate(".op_save","click",function(e){
                        ...
                        if(cansubmit){
                            ...
                            $.ajax({
                                type:"POST",
                                url:"{url_path('taizhang_house','add')}",
                                data : {
                                    isajax:"1",
