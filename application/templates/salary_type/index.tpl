                ...
                <form action="{url_path('salary_type')}" method="get" name="searchform">
                    ...
                    <ul>
                        <li>
                            ...
                            {auth name="salary_type+add"}<a class="addlink" href="{url_path('salary_type','add')}">添加薪资名称</a>{/auth}
                        </li>
                     </ul>
                </form>
            </div>
                ...
                <table class="table">
                    ...
                    <tbody>
                        ...
                        <tr id="row_{$item['id']}">
                           ...
                           <td>
                               {if $item['status'] != '已删除'}
                               {auth name="salary_type+edit"}<a href="{url_path('salary_type','edit','id=')}{$item['id']}">编辑</a>{/auth}
                               {auth name="salary_type+delete"}<a href="javascript:void(0);" data-title="{$item['name']|escape}" data-href="{url_path('salary_type','delete','id=')}{$item['id']}" data-id="{$item['id']}" class="delete">删除</a>{/auth}
                               {/if}
                            </td>
                        </tr>
                        {foreachelse}
