{include file="common/main_header.tpl"}
            <div class="searchform row-fluid">
                <form action="{url_path('reports_reg','index')}" method="post" name="searchform" target="post_iframe">
                    <input type="hidden" value="reports_reg" name="{config_item('controller_trigger')}"/>
                        ...
                        <tr>
                            ...
                            <td>
                                {foreach from=$projectTypeList key=key item=item}
                                    <label><input type="checkbox" name="category[]" value="{$item}" {if $item == $smarty.const.TAIZHANG_WF || $item == $smarty.const.TAIZHANG_PERSON || $item == $smarty.const.TAIZHANG_SH }{else}checked="checked"{/if}/>{$item}</label>
                                {/foreach}
                            </td>
                        </tr>
