{include file="common/main_header.tpl"}
          {include file="common/ke.tpl"}
            <div class="row-fluid">
                {if $action == 'edit'}
                <form action="{url_path('culture','edit')}" method="post" name="infoform">
                    <input type="hidden" name="id" value="{$info['id']}"/>
                {else}
                <form action="{url_path('culture','add')}" method="post" name="infoform">
                {/if}
                    <table class="maintain">
                        <tbody>
                        <tr>
                            <td><label class="required"><em>*</em><strong>标题</strong></label></td><td><input type="text" style="width:500px" name="title" value="{$info['title']}" placeholder="请输入标题"/>{form_error('title')}</td>
                        </tr>
                        <tr>
                ...
                <script>
                    ...
                    {if $feedback == 'success' && $action != 'edit'}
                        if(confirm('{$feedMessage}')){
                            location.href = "{url_path('culture','add')}";
                        }else{
                            location.href = "{url_path('culture')}";
                        }
                    {/if}
